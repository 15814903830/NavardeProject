...
public class OkHttpUtils {
     ...
     * 参数三：请求回调
     */
    public static void doPostXml(String url, String xmlData, final HttpCallBack callback, final int requestId) {
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/xml; charset=utf-8"), xmlData);
        Request request = new Request.Builder().url(url).post(requestBody).build();
        Call call = getInstance().newCall(request);
        call.enqueue(new Callback() {
            @Override
